...
class Cart
{
    ...
    protected function _insert($items = [])
    {
        ...
        if ( ! isset($items[ 'id' ], $items[ 'qty' ], $items[ 'name' ], $items[ 'brand' ]) ) {
        ...
        if ( $this->productNameSafe && ! preg_match('/^[' . $this->productNameRules . ']+$/i' . ( true ? 'u' : '' ), $items[ 'brand' ]) ) {
            log_message('error', 'An invalid name was submitted as the product name: ' . $items[ 'brand' ] . ' The name can only contain alpha-numeric characters, dashes, underscores, colons, and spaces');
            return false;
        }
    ...
    protected function _update($items = []): bool
    {
        ...
        foreach ( array_diff($keys, [ 'id', 'brand']) as $key ) {
    ...
    protected function saveCart(): bool
    {
        ...
        foreach ( $this->cartContents as $key => $val ) {
            ...
            if ( ! is_array($val) || ! isset($val[ 'name' ], $val[ 'qty' ]) ) {
                continue;
            }
